...
<div class="row row-sm">
    <div class="col-xl-12">
        <div class="card">
            ...
            <div class="card-body">
                <div id="table_data">
                    @include('office.requests.preview_request_pagination')
                </div>
            </div>
 ...
 {{-- request evants  --}}
    ...
    <script>
       ...
       $(document).on('click', '.send_email_client', function(e) {
            ...
            console.log(data);
            $.ajax({
                url: "{{route('office.chang_state_request.preview')}}",
                data:data,
       ...
       $(document).on('click', '.close_send_email_client', function(e) {
        ...
        $(document).on('click', '.close_show_email_client', function(e) {
         ...
         $(document).on('click', '#delete_preview_btn', function(e) {

            e.preventDefault();

            console.log( $('#delete_preview_btn').data('id'))
            ...
            const swalWithBootstrapButtons = Swal.mixin({
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        if (result.isConfirmed) {
                            var id = $('#delete_preview_btn').data('id')
                            var url = '{{ route('office.delete_preview.requests',':id') }}';
                            url = url.replace(':id', id);
                            $.ajax({
                                type: "GET",
                                url: url,
