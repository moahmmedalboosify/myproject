		...
		init: function() {
			...
			if( this.settings.autoButtons ) {
				this.$steps.each(function(i, el) {
					...
					if( i > 0 ) {
						$content.append('<a href="#" class="'+mthis.settings.autoButtonsPrevClass+'" data-acc-btn-prev>رجوع</a>');
					}

					// Add next, submit on last
					if( i < ( mthis.$steps.length - 1 ) ) {
						$content.append('<a href="#" class="'+mthis.settings.autoButtonsNextClass+'" data-acc-btn-next>التالي</a>');
